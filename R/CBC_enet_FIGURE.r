...
if(is.null(features.in)){
    ...
    abs(DF.model$coef.mean) > max(abs(DF.null$coef.mean[!is.na(DF.model$coef.mean)]), na.rm=T) & DF.model$coef.pval < 0.05
} else {
...
mg = grid.arrange(arrangeGrob(g1,g2, layout_matrix = lay))
fn.fig = file.path(dn.fig, sprintf("eNet_%s_selected", albl))
ggsave(file=paste0(fn.fig, ".png"), plot=mg, w=6, h=3)
ggsave(file=paste0(fn.fig, ".pdf"), plot=mg, w=6, h=3, useDingbats=FALSE)
